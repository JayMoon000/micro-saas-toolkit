...
<head>
  ...
  <title>AI Background Remover - StarSign16 Toolkit</title>
  <meta name="description" content="100% on-device AI background remover. Free client-side image cutout tool.">
  <link rel="canonical" href="https://tool.starsign16.com/background-remover">
  
  <meta property="og:title" content="AI Background Remover - StarSign16 Toolkit">
  <meta property="og:description" content="100% on-device AI background remover. Free client-side image cutout tool.">
  <meta property="og:url" content="https://tool.starsign16.com/background-remover">
...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans selection:bg-cyan-500 selection:text-white">
  ...
  <header class="border-b border-slate-800 bg-slate-900/50 backdrop-blur sticky top-0 z-40">
    <div class="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
      <a href="/" class="text-sm font-medium text-slate-400 hover:text-cyan-400 transition-colors flex items-center gap-1.5">
        &larr; StarSign16 Toolkit Hub
      </a>
      <a href="/ko/background-remover" class="text-xs px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors">
        한국어
      </a>
    </div>
  </header>
  ...
  <main class="flex-1 max-w-4xl mx-auto px-4 py-8 w-full flex flex-col items-center">
    <div class="text-center mb-8">
      <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-cyan-950/60 border border-cyan-800/60 text-cyan-400">Image & Media</span>
      <h1 class="text-2xl sm:text-3xl font-bold mt-2 text-slate-100">AI Background Remover</h1>
      <p class="text-sm text-slate-400 mt-1">Universal high-precision background removal. 100% on-device client processing.</p>
    </div>
    ...
    <label for="fileInput" id="dropZone" class="w-full max-w-2xl border-2 border-dashed border-slate-700 hover:border-cyan-500 bg-slate-900/40 hover:bg-slate-900/70 rounded-2xl p-8 text-center cursor-pointer transition-all flex flex-col items-center justify-center min-h-[220px]">
      <input type="file" id="fileInput" class="hidden" accept="image/png, image/jpeg, image/webp" aria-label="Select image file">
      ...
      <p class="text-sm font-medium text-slate-200 pointer-events-none">Click or drag & drop an image here</p>
      <p class="text-xs text-slate-500 mt-1 pointer-events-none">Supports PNG, JPG, WEBP (No server upload, 100% Free)</p>
    </label>
    ...
    <div id="resultArea" class="w-full max-w-2xl mt-6 hidden flex-col items-center">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full mb-6">
        <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3 flex flex-col items-center">
          <span class="text-xs text-slate-400 mb-2">Original Image</span>
        ...
        <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3 flex flex-col items-center">
          <span class="text-xs text-cyan-400 mb-2">Removed Background</span>
  ...
  <div id="loadingOverlay" class="hidden fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl flex flex-col items-center">
      ...
      <h3 id="loadingStatusText" class="text-base font-semibold text-slate-100 mb-1">Loading AI model into memory...</h3>
      <p class="text-xs text-slate-400 mb-4">Initial run caches ~25MB model in browser memory.</p>
      <div class="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden mb-3">
        <div id="progressBar" class="bg-gradient-to-r from-cyan-500 to-indigo-500 h-full w-1/4 transition-all duration-300"></div>
      </div>
      <span class="text-[11px] text-slate-500">🔒 100% Client-Side Processing</span>
    </div>
  </div>
  ...
  <script type="module">
    ...
    const msgs = [
      ...
      "Analyzing foreground contours...",
      "Refining alpha transparency pixels...",
      "Almost ready!"
    ];
    let msgTimer = null;
    ...
    function showLoading() {
      ...
      msgTimer = setInterval(() => {
        ...
        progressBar.style.width = `${Math.min(90, (idx + 1) * 25)}%`;
      }, 2000);
    }
    ...
    async function handleImage(file) {
      ...
      try {
        ...
        alert('Failed to remove background: ' + (err.message || 'Processing error.'));
      } finally {
        hideLoading();
      }
    }
...
def run():
    ...
    print("✅ Fixed English messages: background-remover.html")
