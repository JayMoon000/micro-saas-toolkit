...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col font-sans selection:bg-cyan-500 selection:text-white">
  ...
  <script type="module">
    ...
    ['dragenter', 'dragover'].forEach(eventName => {
      dropZone.addEventListener(eventName, () => dropZone.classList.add('border-cyan-500', 'bg-slate-900/70'), false);
    });

    ['dragleave', 'drop'].forEach(eventName => {
      dropZone.addEventListener(eventName, () => dropZone.classList.remove('border-cyan-500', 'bg-slate-900/70'), false);
    });
    ...
    async function handleImage(file) {
      if (!file || !file.type.startsWith('image/')) {
        alert('이미지 파일(PNG, JPG, WEBP 등)만 지원합니다.');
        return;
      }
      ...
      try {
        ...
        alert('배경 제거 실패: ' + (err.message || '브라우저 호환성 에러'));
      } finally {
        hideLoading();
      }
    }
...
EN_HTML = KO_HTML.replace("AI 배경 제거 (누끼 따기) - StarSign16 Toolkit", "AI Background Remover - StarSign16 Toolkit") \
                 ...
                 .replace("이미지 파일(PNG, JPG, WEBP 등)만 지원합니다.", "Please select an image file.") \
...
def run():
    ...
    print("✅ Fixed Drag/Drop & Click Events: ko/background-remover.html")

    with open(en_path, "w", encoding="utf-8") as f:
        f.write(EN_HTML.strip())
    print("✅ Fixed Drag/Drop & Click Events: background-remover.html")
